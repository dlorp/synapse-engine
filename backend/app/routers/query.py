...
async def auto_select_debate_participants(
    model_selector: ModelSelector
) -> list[str]:
...
async def _call_model_direct(
    ...
    try:
        ...
        return {
            ...
            "usage": {
                "total_tokens": result.get("tokens_predicted", 0) + result.get("tokens_evaluated", 0)
            }
        }
    finally:
        await client.close()
        ...
        try:
            ...
            if index_path.exists() and metadata_path.exists():
                ...
                logger.info(
                    ...
                    extra={"query_id": query_id, "artifacts_count": len(cgrag_artifacts)}
                )
...
async def _process_debate_mode(
    ...
    logger.info(f"⚔️ Debate participants: {participants[0]} (PRO) vs {participants[1]} (CON)")
    ...
    if request.council_moderator:
        ...
        if request.council_moderator_model:
            ...
            logger.info(f"Active moderator enabled with user-specified model: {moderator_model_for_interjections}")
        else:
            ...
            if model_selector:
                try:
                    ...
                    logger.info(f"Active moderator enabled with auto-selected model: {moderator_model_for_interjections}")
                except Exception as e:
                    logger.warning(f"Failed to auto-select moderator model: {e} - disabling active moderation")
                    moderator_model_for_interjections = None

    # =================================================================
    # RUN DIALOGUE ENGINE
    # =================================================================
    logger.info(f"Starting debate dialogue: max {request.council_max_turns or 10} turns, dynamic_termination={request.council_dynamic_termination}")
    ...
    try:
        dialogue_result = await dialogue_engine.run_debate_dialogue(
            ...
            enable_active_moderator=request.council_moderator_active and moderator_model_for_interjections is not None,
    ...
    logger.info(f"✅ Debate dialogue completed: {len(dialogue_result.turns)} turns, {dialogue_result.termination_reason}, {total_time}ms")
    ...
    try:
        ...
        if query_mode == "two-stage":
            ...
            try:
                ...
                logger.info(f"✅ Stage 1 complete: {len(stage1_response)} chars in {stage1_time_ms}ms")
            except Exception as e:
...
Refined Response:"""
            ...
            # ================================================================
            logger.info(f"🔬 Benchmark mode: {'serial' if request.benchmark_serial else 'parallel'}")
            ...
            if not enabled_models:
                raise HTTPException(
                    status_code=503,
                    detail="No enabled models available for benchmark"
                )
            ...
            if effective_web_search:
                try:
                    ...
                    logger.warning(f"⚠️ Web search failed for benchmark query {query_id}: {e}")
                ...
                for model_id in enabled_models:
                    ...
                    try:
                        ...
                        logger.info(
                            ...
                            extra={"query_id": query_id, "model_id": model_id, "time_ms": response_time_ms}
                        )
