            ...
            "content": {
                "application/json": {
                    "example": {
                        ...
                        "totalQueries": 450,
                        "avgLatencyMs": 2350.5,
                        "tierDistribution": {"Q2": 300, "Q3": 100, "Q4": 50},
                    }
                }
            },
        }
...
async def get_tier_metrics() -> TierMetricsResponse:
    ...
    logger.debug("Tier metrics retrieved", extra={"tier_count": len(metrics.tiers)})
    ...
    responses={
        200: {
            ...
            "content": {
                "application/json": {
                    "example": {
                        "vram": {"used": 8.5, "total": 24.0, "percent": 35.4},
                        "cpu": {"percent": 45.2, "cores": 16},
                        "memory": {"used": 12.3, "total": 32.0, "percent": 38.4},
                        "faissIndexSize": 524288000,
                        "redisCacheSize": 104857600,
                        "activeConnections": 5,
                        "threadPoolStatus": {"active": 4, "queued": 2},
                        "diskIO": {"readMBps": 15.3, "writeMBps": 8.7},
                        "networkThroughput": {"rxMBps": 2.5, "txMBps": 1.8},
                    }
                }
            },
        }
                ...
                "application/json": {
                    "example": {
                        ...
                        "modelAvailability": [
                            ...
                            {"tier": "Q3", "available": 2, "total": 2},
                            {"tier": "Q4", "available": 1, "total": 1},
                        ],
                    }
                }
            },
...
async def get_context_utilization() -> ContextUtilization:
    ...
    try:
        ...
        tokens_total = (
            total_allocations * default_context_size
        ...
        logger.debug(
            ...
            extra={"percentage": avg_utilization, "active_queries": total_allocations},
        )
        ...
        return ContextUtilization(
            percentage=0.0, tokens_used=0, tokens_total=8192, active_queries=0
        )
