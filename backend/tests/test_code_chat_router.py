...
class TestCodeChatRequestModel:
    ...
    def test_valid_request(self):
        ...
        request = CodeChatRequest(
            query="Read the main.py file", workspace_path="/workspace/project"
        )
    ...
    def test_request_defaults(self):
        ...
        request = CodeChatRequest(query="test", workspace_path="/workspace")
    ...
    def test_request_validation_max_iterations(self):
        ...
        # Valid within bounds
        request = CodeChatRequest(
            query="test", workspace_path="/workspace", max_iterations=1
        )
        assert request.max_iterations == 1

        request = CodeChatRequest(
            query="test", workspace_path="/workspace", max_iterations=50
        )
...
class TestWorkspaceModels:
    ...
    def test_workspace_validation_valid(self):
        ...
        validation = WorkspaceValidation(
            valid=True, is_git_repo=True, file_count=150, has_cgrag_index=True
        )
    ...
    def test_workspace_validation_invalid(self):
        ...
        validation = WorkspaceValidation(valid=False, error="Path does not exist")
...
class TestWorkspacesEndpoint:
    ...
    def mock_services(self):
        ...
        with (
            patch("app.routers.code_chat.validate_path") as mock_validate,
    ...
    async def test_query_validates_workspace(self):
        ...
        with patch("app.routers.code_chat.validate_path") as mock_validate:
            ...
            request_obj = CodeChatRequest(
                query="test query", workspace_path="../../../etc"
            )
...
class TestCancelEndpoint:
    ...
    async def test_cancel_session(self):
        ...
        with patch(
            "app.routers.code_chat.get_agent", new_callable=AsyncMock
        ) as mock_get:
