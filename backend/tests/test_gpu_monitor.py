    ...
    def test_get_nvidia_metrics_success(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0, stdout="NVIDIA GeForce RTX 4090, 8192, 24576, 45\n"
            )
...
class TestAppleSiliconMetrics:
    ...
    def test_get_apple_silicon_metrics_success(self):
        ...
        with (
            patch("app.services.gpu_monitor.psutil") as mock_psutil,
            patch("subprocess.run") as mock_run,
        ):
            mock_psutil.virtual_memory.return_value = mock_memory

            mock_run.return_value = MagicMock(returncode=0, stdout="Apple M4 Pro\n")
